...
int main() {
   // Functions *frt = Functions::get_function("legal(xplayer, mark(X,Y))");
    ...
    vector<string> input_file = {"init(cell(1, 1, x))",
                                 ...
                                 "init(cell(3, 2, b))",
                                 "init(cell(3, 3, b))",
                                 "row(Player, X) :- (cell(X + 2, 1, Player) & cell(X / 2 + 1, 2, Player) & cell(X, 3, Player)) | (cell(1, X, Player) & cell(2, X, Player) & cell(3, X, Player))",
    ...
    cout << Functions::evaluate("row(x, 1)");
}