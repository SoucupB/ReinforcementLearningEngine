...
int main(int argc, char *argv[]) {
  ...
  if(argument == "train") {
    string first_folder = Utils::get_path_from_cfg("3x3_path_first_train");
    string second_folfer = Utils::get_path_from_cfg("3x3_path_second_train");
    BattleModel<FirstPlayer, SecondPlayer, State, FirstAction, FirstAction>::tournament_learning(600, 5000, 1000, first_folder,
                                                                                                second_folfer, 3, 6, 42000000, true, {13, 13, 13}, 0.01, true, 0.11, false);
  }
  else {
    string mode = argv[2];
