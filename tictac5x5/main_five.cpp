...
void server(string file_name, string path, string mode, bool player_start) {
    ...
    if(!player_start) {
      if(mode == "hybrid")
        ...
        bot_x.activate_nn();
      bot_x.assign_params(5, path + file_name, {46, 46, 46}, 0.2);
      bot_x.get_bot();
    }
...
int main(int argc, char *argv[]) {
  ...
  if(argument == "train") {
    ...
    if(starter)
      server(argument, Utils::get_path_from_cfg("5x5_path_play_second"), mode, starter);
    else
      server(argument, Utils::get_path_from_cfg("5x5_path_play_first"), mode, starter);
  }
}