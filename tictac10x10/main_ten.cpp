...
void server(string file_name, string path, string mode, bool player_start) {
    ...
    if(player_start) {
      if(mode == "hybrid")
        ...
        bot_o.activate_nn();
      bot_o.assign_params(5, path + file_name, {41, 41, 41}, 0.2);
      bot_o.get_bot();
    }
    ...
    if(!player_start) {
      if(mode == "hybrid")
        ...
        bot_x.activate_nn();
      bot_x.assign_params(5, path + file_name, {41, 41, 41}, 0.2);
      bot_x.get_bot();
    }
...
int main(int argc, char *argv[]) {
  ...
  if(argument == "train") {
    string first_folder = Utils::get_path_from_cfg("10x10_path_first_train");
    string second_folfer = Utils::get_path_from_cfg("10x10_path_second_train");
    BattleModel<FirstPlayer, SecondPlayer, State, FirstAction, FirstAction>::tournament_learning(500, 50000, 500, first_folder,
                                                                                                second_folfer, 5, 6, 42000000, true, {41, 41, 41}, 0.01, true, 0.11, false);
  }
  else {
    string mode = argv[2];
    ...
    if(starter)
      server(argument, Utils::get_path_from_cfg("10x10_path_play_second"), mode, starter);
    else
      server(argument, Utils::get_path_from_cfg("10x10_path_play_first"), mode, starter);
  }
}