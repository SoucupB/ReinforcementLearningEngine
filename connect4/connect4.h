...
class State {
public:
  vector<char> vecs_hash{vector<char>(42,0)}, vecs_neural_net{vector<char>(42,0)};
  ...
  vector<char> state_to_vec_hash() {
    return this->vecs_hash;
  }
  vector<char> state_to_vec_neural() {
    return this->vecs_neural_net;
  }
};
...
class FirstPlayer {
   ...
   public:
      ...
      vector<FirstAction> get_all_actions(State &state) {
        ...
        for(int i = 0; i < 6; i++) {
          for(int j = 0; j < 7; j++) {
            if((i == 5 && state.game[i][j] == 0) || (state.game[i][j] == 0 && state.game[i + 1][j] != 0)) {
              FirstAction action;
              action.y = i;
              action.x = j;
              action.id = this->id;
